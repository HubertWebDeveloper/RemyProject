...
if(isset($_POST['filter'])){
    ...
    if (!empty($location) && !empty($price) && !empty($size)) {
        ...
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE location LIKE '%$location%' AND new_price <= '$price' AND description LIKE '%$size%' AND status='Posted'");
        $loca = $location;
        $pri = $price;
        $siz = $size;
    } elseif (!empty($location) && !empty($price)) {
        // Location and price are set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE location LIKE '%$location%' AND new_price <= '$price' AND status='Posted'");
        $loca = $location;
        $pri = $price;
    } elseif (!empty($location) && !empty($size)) {
        // Location and size are set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE location LIKE '%$location%' AND description LIKE '%$size%' AND status='Posted'");
        $loca = $location;
        $siz = $size;
    } elseif (!empty($price) && !empty($size)) {
        // Price and size are set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE new_price <= '$price' AND description LIKE '%$size%' AND status='Posted'");
        $pri = $price;
        $siz = $size;
    } elseif (!empty($location)) {
        // Only location is set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE location LIKE '%$location%' AND status='Posted'");
        $loca = $location;
    } elseif (!empty($price)) {
        // Only price is set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE new_price <= '$price' AND status='Posted'");
        $pri = $price;
    } elseif (!empty($size)) {
        // Only size is set
        $cards = mysqli_query($con, "SELECT * FROM cards WHERE description LIKE '%$size%' AND status='Posted'");
        $siz = $size;
    } else {
        // No fields set
    ...
    <div class="container">
        ...
        <div class="events-header text-center">
            ...
            if(isset($_POST['filter'])){
                ...
                ?><h2>All Vacant Houses And B&Bs.</h2><?php
            }
            ?>
